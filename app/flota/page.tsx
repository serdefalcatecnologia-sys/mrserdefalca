...
export default function PanelFlotaOperaciones() {
  ...
  // ESTADOS DE RUTAS
  const [relojVisual, setRelojVisual] = useState('');
  ...
  useEffect(() => {
    ...
    actualizarReloj();
    const intervaloReloj = setInterval(actualizarReloj, 60000);
  ...
  const cargarDatosCompletos = async (vehiculoAsignado?: string) => {
    const [resRutas, resVehiculos] = await Promise.all([
      supabase.from('flota_rutas').select('*').order('fecha', { ascending: false }).limit(50),
  ...
  const registrarReporteRuta = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('flota_rutas').insert([{
        ...
        municipio: municipio,
        ruta: rutaSector,
        toneladas: parseFloat(tonelaje) || 0,
        estatus: estadoRuta,
                ...
                <form onSubmit={registrarReporteRuta} className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      value={relojVisual} 
                      readOnly 
                      title="La fecha y HORA exactas se guardarán automáticamente por seguridad al registrar el viaje."
                      className="w-full rounded-lg border border-zinc-200 bg-zinc-100 p-2.5 text-sm font-bold text-emerald-700 outline-none cursor-not-allowed dark:bg-zinc-800/80 dark:border-zinc-700" 
                    />
                  </div>
                    ...
                    <tbody className="divide-y divide-zinc-200">
                      {reportes.map((rep) => (
                        <tr key={rep.id} className="hover:bg-zinc-50 transition-colors">
                          <td className="px-6 py-3 font-medium whitespace-nowrap">{formatearFechaHora(rep.fecha)}</td>
                          <td className="px-6 py-3 font-bold text-zinc-900">{rep.unidad}</td>
                          <td className="px-6 py-3">
                            <p className="text-zinc-800 font-medium">{rep.municipio}</p>
                            <p className="text-xs text-zinc-500">{rep.ruta}</p>
                          </td>
                          <td className="px-6 py-3 text-center font-bold text-emerald-600">
                            {rep.toneladas} Tn
                          </td>
                          <td className="px-6 py-3">{rep.estatus}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
