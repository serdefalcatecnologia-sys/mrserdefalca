...
export default function VistaFlotaRutas() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        if (error) {
          console.error("Error obteniendo datos de la flota:", error);
        } else {
          setDatosFlota(data || []);
        }
      } catch (err) {
  ...
  const datosFiltrados = datosFlota.filter(item => {
    ...
    const cumpleEstado = estadoFiltro === '' || item.estado === estadoFiltro;
  ...
  const vehiculosOperativos = datosFiltrados.filter(item => item.estado === 'Operativo').length;
  const vehiculosMantenimiento = datosFiltrados.filter(item => item.estado === 'Mantenimiento' || item.estado === 'Inactivo').length;
  ...
  const generarPDF = () => {
    ...
    // Configurar tabla (Agregamos Conductor)
    const columnas = ["N° Ruta", "Vehículo", "Conductor", "Municipio", "Tonelaje", "Estado", "Fecha"];
    const filas = datosFiltrados.map(item => [
      item.id,
      item.vehiculo,
      item.conductor || 'No asignado',
      item.municipio,
      `${Number(item.tonelaje || 0).toFixed(2)} T`,
      item.estado,
      item.fecha
    ]);
  ...
  return (
    <div className="min-h-screen bg-zinc-50 p-8 dark:bg-zinc-950 relative">
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          <p className="text-sm font-medium text-zinc-500">Rutas Operativas</p>
      ...
      <div className="mb-6 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 flex flex-col gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex-1">
          <label className="mb-1 block text-xs font-medium text-zinc-500">Estado del Vehículo</label>
          ...
          {cargando ? (
            ...
            <table className="w-full text-left text-sm text-zinc-600 dark:text-zinc-400">
              <thead className="bg-zinc-50 text-xs uppercase text-zinc-500 dark:bg-zinc-800/50">
                <tr>
                  <th className="px-6 py-4 font-semibold">N° Ruta</th>
              ...
              <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                {datosFiltrados.map((item, index) => (
                  <tr key={index} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">{item.vehiculo} <br/><span className="text-xs text-zinc-400">{item.placa}</span></td>
                    <td className="px-6 py-4">{item.conductor || 'No asignado'}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium
                        ${item.estado === 'Operativo' ? 'bg-emerald-100 text-emerald-800' : ''}
                        ${item.estado === 'Mantenimiento' ? 'bg-amber-100 text-amber-800' : ''}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-zinc-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">N° de Ruta</p>
                  <p className="mt-1 text-base text-zinc-900 dark:text-zinc-100 font-medium">{registroSeleccionado.id}</p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Vehículo / Placa</p>
                  <p className="mt-1 text-base text-zinc-900 dark:text-zinc-100">{registroSeleccionado.vehiculo} <span className="text-zinc-400 text-sm">({registroSeleccionado.placa})</span></p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Conductor Asignado</p>
                  <p className="mt-1 text-base text-zinc-900 dark:text-zinc-100">{registroSeleccionado.conductor || 'No asignado'}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">Municipio Destino</p>
                ...
                <div>
                  ...
                  <p className="mt-1 text-base font-bold text-emerald-600">{Number(registroSeleccionado.tonelaje || 0).toFixed(2)} T</p>
                </div>
              </div>

              {/* ALERTA DE MOTIVO DE INOPERATIVIDAD */}
              {(registroSeleccionado.estado === 'Inactivo' || registroSeleccionado.estado === 'Mantenimiento') && (
                <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 rounded-lg">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wider">Vehículo Detenido</p>
                      <p className="mt-1 text-sm text-red-800 dark:text-red-300">
                        <span className="font-semibold">Motivo:</span> {registroSeleccionado.motivo_inoperativo || 'Motivo no especificado por el administrador.'}
                      </p>
                    </div>
                  </div>
                </div>
